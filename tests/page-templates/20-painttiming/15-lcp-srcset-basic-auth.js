...
describe("e2e/20-painttiming/15-lcp-srcset-basic-auth", function() {
  var tf = BOOMR.plugins.TestFramework;
  var t = BOOMR_test;
  var locPrefix = window.location.protocol + "//" + window.location.hostname + ":" + window.location.port;
  ...
  it("Should have exposed LCP metric src (pt.lcp.src) (if LargestContentfulPaint is supported and happened by load)", function() {
    ...
    assert.include(tf.lastBeacon()["pt.lcp.src"], "/pages/20-painttiming/support/img.jpg");
  });

  it("Should NOT have exposed LCP metric src Basic Auth (pt.lcp.src) (if LargestContentfulPaint is supported and happened by load)", function() {
    if (!t.isLargestContentfulPaintSupported()) {
      return this.skip();
    }
    ...
    assert.notInclude(tf.lastBeacon()["pt.lcp.src"], "test:test");
  });
  ...
  it("Should have exposed LCP metric ID (pt.lcp.id) (if LargestContentfulPaint is supported and happened by load)", function() {
    ...
    assert.equal(tf.lastBeacon()["pt.lcp.id"], "lcp-elem");
  });
  ...
  it("Should have exposed LCP metric Pseudo-CSS Selector (pt.lcp.e) (if LargestContentfulPaint is supported and happened by load)", function() {
    ...
    assert.equal(tf.lastBeacon()["pt.lcp.e"], "img#lcp-elem");
  });
  ...
  it("Should have exposed LCP metric src-set (pt.lcp.srcset) (if LargestContentfulPaint is supported and happened by load)", function() {
    ...
    assert.equal(b["pt.lcp.srcset"], locPrefix + "/pages/20-painttiming/support/img.jpg?id=1w 1w," + locPrefix + "/pages/20-painttiming/support/img.jpg?id=9999w 9999w");
  });

  it("Should have NOT exposed HTTP Basic Auth in LCP metric src-set (pt.lcp.srcset) (if LargestContentfulPaint is supported and happened by load)", function() {
    var b = tf.beacons[0];
    ...
    assert.equal(b["pt.lcp.srcset"], BOOMR.plugins.PaintTiming.metrics.lcpSrcset());
    assert.notInclude(b["pt.lcp.srcset"], "test:test@");
  });
