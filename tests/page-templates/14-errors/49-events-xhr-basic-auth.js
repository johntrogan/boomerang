...
describe("e2e/14-errors/49-events-xhr-basic-auth", function() {
  ...
  it("Should have fileName of the script (if set)", function() {
    var b = tf.lastBeacon();
    var err = BOOMR.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err))[0];

    if (err.fileName) {
      assert.include(err.fileName, "trigger-xhr-error.js");
    }
    else {
      return this.skip();
    }
  ...
  it("Should have fileName of the script without the auth (if set)", function() {
    var b = tf.lastBeacon();
    var err = BOOMR.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err))[0];

    if (err.fileName) {
      assert.notInclude(err.fileName, "test:test@");
    }
    else {
      return this.skip();
    }
  ...
  it("Should have lineNumber ~ 9", function() {
    var b = tf.lastBeacon();
    var err = BOOMR.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err))[0];

    if (err.lineNumber) {
      assert.closeTo(err.lineNumber, 9, 1);
    }
    else {
      return this.skip();
    }
